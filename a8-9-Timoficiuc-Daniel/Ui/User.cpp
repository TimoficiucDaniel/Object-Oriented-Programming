...
void User::show_menu() {
    ...
    std::cout<<"4 - display adoption list. \n";
    std::cout<<"********************************\n";
}
...
void User::run() {
    ...
    while(x) {
        try {
            ...
            v.validateOptionUser(intOption);
            if (intOption == 0)
                x = false;
            else if (intOption == 1)
                this->first_option();
    ...
    this->userServ.database->storeToFile();
}
/**
 * function that lists all elements in repo
 */
void User::listAll() {
    if (this->userServ.getSize() == 0)
        std::cout << "Empty database.\n";
    for(int i = 0; i < this->userServ.getSize(); i++)
    {
        std::cout << this->userServ.getDog(i).toString();
    }

}
/**
 * function corresponding to first functionality that parses through objects in repo and lets you adopt them
...
Dog User::show_next_dog() {
    ...
    return dog;
    } catch (RepoIteratorException& re){
        std::cout<<re.what();
    }
}
/**
...
void User::second_option() {
    ...
    while (x) {
        ...
        if (intOption == 0)
            ...
            } else if (intOption == 2 && ok) {
                ok = false;
                this->adopt(dog);
                if (this->serv.database->getSize() == 0) {
                    x = false;
                }
            } else if (intOption == 3 && ok) {
                dog.open();
            } else {
