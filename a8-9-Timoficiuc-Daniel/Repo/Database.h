...
template <typename TElem> class Database{
    ...
    std::string filename;
        ...
        void update(const int id, const int option, std::string string) {
            int index = 0;
            for (auto i : this->elements) {
                if (this->elements[index].getId() == id) {
                    if (option == 1)
                        this->elements[index].setName(string);
                    if (option == 2)
                        this->elements[index].setBreed(string);
                    if (option == 4)
                        this->elements[index].setLink_to_photo(string);
                    if (option == 3) {
                        for (char const &c: string) {
                            if (std::isdigit(c) == 0)
                                throw RepoException("Age has to be an integer.\n");
                        }
                        int age = stoi(string);
                        this->elements[index].setAge(age);
      ...
      */
        ...
        std::ofstream file("C:/Users/timod/CLionProjects/A8-9/Repo/text.txt");
        if(!file.is_open())
            return;
        //throws exception
        for(auto d : this->elements)
         ...
         */
        void readFromFile(){
            std::ifstream file("C:/Users/timod/CLionProjects/A8-9/Repo/text.txt");
